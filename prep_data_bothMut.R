startTime <- Sys.time()
cat(paste0("> Rscript prep_data_bothMut.R\n"))

# Rscript prep_data_bothMut.R
...
# Rscript prep_data_bothMut.R <cancer type> <cond1> <cond1_out> <cond2> <cond2_out>
# Rscript prep_data_bothMut.R THCA mutBRAF mutBRAF mutRAS mutRAS
...
stopifnot(grepl("^mut.+", cond1))
stopifnot(grepl("^mut.+", cond2))

# use this script only if both cond are mutations
...
curr_gamDT1 <- curr_gamDT[myalterations1,]
stopifnot(nrow(curr_gamDT1) == length(myalterations1))

curr_gamDT2 <- curr_gamDT[myalterations2,]
