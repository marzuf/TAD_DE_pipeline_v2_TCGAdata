...
outputExprFile <- file.path(setDir, "/mnt/ed4/marie/other_datasets", "TCGA_vAll", cmp_name, "rnaseqDT_v2.Rdata")
outputSamp1File <- file.path(setDir, "/mnt/ed4/marie/other_datasets", "TCGA_vAll", cmp_name,  paste0(cond1_out, "_ID.Rdata"))
outputSamp2File <- file.path(setDir, "/mnt/ed4/marie/other_datasets",  "TCGA_vAll",cmp_name, paste0(cond2_out, "_ID.Rdata"))
...
if(grepl("^mut.+", cond1) & grepl("^mut.+", cond2)) {
  ...
  txt <- paste0("... for alterations in mut1\t=\t", mymut1, "\n" )
  txt <- paste0("... for alterations in mut2\t=\t", mymut2, "\n" )
  printAndLog(txt, logFile)
  
  if(mutType == "symbol") {
    stopifnot(mymut1 %in% gamEntrez2symb)
    mymut_entrez1 <- names(gamEntrez2symb[gamEntrez2symb == mymut1])
    stopifnot(mymut2 %in% gamEntrez2symb)
    mymut_entrez2 <- names(gamEntrez2symb[gamEntrez2symb == mymut2])
    stopifnot(length(mymut_entrez1) == 1)
    stopifnot(length(mymut_entrez2) == 1)
  } else if(mutType == "entrez") { 
    stopifnot(mymut1 %in% names(gamEntrez2symb))
    mymut_entrez1 <- mymut1
    stopifnot(mymut2 %in% names(gamEntrez2symb))
    mymut_entrez2 <- mymut2
  } else {
    stop("-- error -- \n")
  }
  
  stopifnot(mymut_entrez1 %in% names(gamAlterationsByEntrez))
  myalterations1 <- gamAlterationsByEntrez[[paste0(mymut_entrez1)]]
  
  stopifnot(mymut_entrez2 %in% names(gamAlterationsByEntrez))
  myalterations2 <- gamAlterationsByEntrez[[paste0(mymut_entrez2)]]
  
  if(mutType == "symbol") {
    myalterations1_v2 <- names(unlist(Filter(function(x) mymut1 %in% x , alterations$alteration_gene)  ))
    stopifnot(sort(myalterations1) == sort(myalterations1_v2))
    
    myalterations2_v2 <- names(unlist(Filter(function(x) mymut2 %in% x , alterations$alteration_gene)  ))
    stopifnot(sort(myalterations2) == sort(myalterations2_v2))
  }
  
  #### !!! CHANGED HERE 06.12.2018 => FOCUS ONLY ON MUTATION !!!
  txt <- paste0("!!! take only \"MUT\" alterations !!!\n")
  printAndLog(txt, logFile)
  myalterations1 <- myalterations1[grepl("^MUT\\.", myalterations1)]
  stopifnot(length(myalterations1) == 1)
  myalterations2 <- myalterations2[grepl("^MUT\\.", myalterations2)]
  stopifnot(length(myalterations2) == 1)
  ####
  
  txt <- paste0("... found following alterations for mut1:\t=\t", paste0(myalterations1, collapse=","), "\n")
  printAndLog(txt, logFile)
  
  txt <- paste0("... found following alterations for mut2:\t=\t", paste0(myalterations2, collapse=","), "\n")
  printAndLog(txt, logFile)
  
  stopifnot(myalterations1 %in% rownames(curr_gamDT))
  stopifnot(myalterations2 %in% rownames(curr_gamDT))
  
  curr_gamDT1 <- curr_gamDT[myalterations1,, drop=F]
  stopifnot(nrow(curr_gamDT1) == length(myalterations1))
  
  curr_gamDT2 <- curr_gamDT[myalterations2,, drop=F]
